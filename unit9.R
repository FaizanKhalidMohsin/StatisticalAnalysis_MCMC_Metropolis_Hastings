...
### random MH, generate binomial from uniform 
binom = function (n, nn, p) 
{
        vec = vector("numeric", n)
        vec[1] = 0
        for (i in 2:n) {
                y = round(runif(1, 0, nn ))
                y = y + vec[i-1]
                aprob = min(1, dbinom(y, nn , p )/dbinom(vec[i-1], nn , p ))
                u = runif(1)
                if (u < aprob) 
                    vec[i] = y
                else 
...
nn = 100; pp = .1
binomvec<-binom(10000, nn, pp)
par(mfrow=c(2,1))
plot(ts(binomvec))
hist(binomvec,30)
theoretical_mean = nn*pp; print(theoretical_mean)
theoretical_var = nn*pp*(1-pp); print(theoretical_var)
...
cauchy = function (n) 
{
        ...
        vec[1] = rnorm(1,0,100)
        for (i in 2:n) {
                y = rnorm(1, vec[i-1],100 )
                aprob = min(1, dcauchy(y)/dcauchy(vec[i-1]))
                u = runif(1)
                if (u < aprob) 
                    vec[i] = y
                else 
...
cauchyvec<-cauchy(10000)
par(mfrow=c(3,1))
plot(ts(cauchyvec))
hist(cauchyvec[2000:10000],30)
hist(rcauchy(10000),30)

### independence chain, generate binomial from normal with the same mean and variance 
binom_ind_ch = function (n, nn, p) 
{
   # mu = nn*p
   # sigma = nn*p*(1-p)
   vec = vector("numeric", n)
   ...
   for (i in 2:n) {
      y <- round(runif(1, 0, nn))
      aprob <- min(1, (dbinom(y, nn, p)/dbinom(vec[i-1], nn, p))/(dunif(y, 0, nn)/dnorm(vec[i-1], 0, nn)))
      u <- runif(1, 0, nn)
      if (u < aprob) 
          vec[i] = y
      else 
...
vec<-binom_ind_ch(10000,nn,pp)
par(mfrow=c(2,1))
plot(ts(vec))
hist(vec,20)
theo_mean = nn*pp; theo_mean
theo_var = nn*pp*(1-pp); theo_var
mean(vec)
var(vec)
